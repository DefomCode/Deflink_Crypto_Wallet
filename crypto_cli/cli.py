...
def import_key(
    ...
    if private_key is None:
        private_key = Prompt.ask("Приватный ключ (0x...)", password=True)

    if not adapter.validate_private_key(private_key):
        console.print("[bold red]❌ Невалидный приватный ключ.[/]")
        raise typer.Exit(code=1)
    ...
    try:
        address = import_wallet(name, private_key, password)
...
def show_wallets():
    ...
    caches = get_wallet_caches(network_type=adapter.currency_symbol)

    table = Table(title="💰 Ваши кошельки", show_header=True, header_style="bold cyan")
    table.add_column("Имя", style="green", no_wrap=True)
    table.add_column(f"Адрес ({adapter.currency_symbol})", style="white")
    ...
    for name, address in wallets.items():
        ...
        if cache and cache["balance_eth"] is not None:
            bal_str = f"{cache['balance_eth']:.6f} {adapter.currency_symbol}"
...
def balance(
    ...
    if coin_balance is None:
        ...
        try:
            update_wallet_cache(name, address, coin_balance, coin_balance * usd_price if usd_price else None, network_type=symbol)
        except Exception:
            pass
...
def pay(
    ...
    try:
        add_transaction(tx_hash, from_name, to_address, amount, fee, network_type=symbol)
    except Exception as e:
    ...
    if current_balance is not None and usd_price is not None:
        try:
            ...
            update_wallet_cache(from_name, wallets[from_name], new_balance, new_balance * usd_price, network_type=symbol)
        except Exception:
            pass
...
def history(
    ...
    if pending_hashes:
        ...
        for tx_hash in pending_hashes:
            receipt = adapter.wait_for_receipt(tx_hash, timeout=10, poll_interval=2)
